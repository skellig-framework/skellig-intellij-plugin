...
object CucumberUtil {
    ...
    fun getTheBiggestWordToSearchByIndex(regexp: String): String {
        var result = ""
        var start = 0
        if (regexp.startsWith(org.skellig.plugin.language.feature.CucumberUtil.PREFIX_CHAR)) {
            start += org.skellig.plugin.language.feature.CucumberUtil.PREFIX_CHAR.length
        }
        var end = regexp.length
        if (regexp.endsWith(org.skellig.plugin.language.feature.CucumberUtil.SUFFIX_CHAR)) {
            end -= org.skellig.plugin.language.feature.CucumberUtil.SUFFIX_CHAR.length
        }
    ...
    fun prepareStepRegexp(stepName: String): String {
        var result = stepName
        for (rule in org.skellig.plugin.language.feature.CucumberUtil.ARR) {
    ...
    fun buildRegexpFromCucumberExpression(
        ...
        cucumberExpression = org.skellig.plugin.language.feature.CucumberUtil.escapeCucumberExpression(cucumberExpression)
        cucumberExpression = org.skellig.plugin.language.feature.CucumberUtil.replaceNotNecessaryTextTemplateByRegexp(cucumberExpression)
        cucumberExpression = org.skellig.plugin.language.feature.CucumberUtil.processExpressionOrOperator(cucumberExpression)
        val escapedCucumberExpression = cucumberExpression
        val parameterTypeValues: MutableList<Pair<TextRange, String?>?> = ArrayList()
        org.skellig.plugin.language.feature.CucumberUtil.processParameterTypesInCucumberExpression(escapedCucumberExpression) { range: TextRange ->
    ...
    fun replaceNotNecessaryTextTemplateByRegexp(cucumberExpression: String): String {
        val matcher = org.skellig.plugin.language.feature.CucumberUtil.OPTIONAL_PATTERN.matcher(cucumberExpression)
        val result = StringBuilder()
        while (matcher.find()) {
    ...
    fun isCucumberExpression(stepDefinitionPattern: String): Boolean {
        ...
        org.skellig.plugin.language.feature.CucumberUtil.processParameterTypesInCucumberExpression(stepDefinitionPattern) { textRange: TextRange ->
    ...
    fun escapeCucumberExpression(stepPattern: String): String {
        return org.skellig.plugin.language.feature.CucumberUtil.ESCAPE_PATTERN.matcher(stepPattern).replaceAll("\\\\$1")
    }
    ...
    init {
        val standardParameterTypes = mapOf(
          ...
          kotlin.Pair("string", "\"(?:[^\"\\\\]*(?:\\\\.[^\"\\\\]*)*)\"|'(?:[^'\\\\]*(?:\\\\.[^'\\\\]*)*)'"),
          kotlin.Pair("", "(.*)"))
        org.skellig.plugin.language.feature.CucumberUtil.STANDARD_PARAMETER_TYPES = Collections.unmodifiableMap(standardParameterTypes)
    }
}