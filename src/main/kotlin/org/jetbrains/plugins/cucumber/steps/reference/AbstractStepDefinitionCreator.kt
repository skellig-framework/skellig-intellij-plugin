...
abstract class AbstractStepDefinitionCreator : StepDefinitionCreator {
    ...
    companion object {
        private fun findStepDefinitionDirectory(featureFile: PsiFile): PsiDirectory? {
            ...
            while (featureDir != null &&
                !Comparing.equal(featureDir, contentRoot) && featureDir.findChild(CucumberUtil.STEP_DEFINITIONS_DIR_NAME) == null
            ) {
                featureDir = featureDir.parent
            }
            if (featureDir != null) {
                val stepsDir = featureDir.findChild(CucumberUtil.STEP_DEFINITIONS_DIR_NAME)
                if (stepsDir != null) {
    ...
    override fun getStepDefinitionFilePath(psiFile: PsiFile): String {
        ...
        if (parent != null && CucumberUtil.STEP_DEFINITIONS_DIR_NAME == parent.name) {
            return file.name
        }
        ...
        while (parent != null) {
            ...
            if (CucumberUtil.STEP_DEFINITIONS_DIR_NAME == name) {
                break
            }
